...
class StripeWebhook
{
    ...
    {
        if (config('my_config.stripe_webhook_secret')) {
    ...
    public function test(Request $request){
        ...
        if ($request->user()->hasDefaultPaymentMethod()) {
            $paymentMethod = $request->user()->defaultPaymentMethod()->toArray();
            ...
            //dump("No default payment gateway");
            return view('teckiproadmin::checkout/stripe/recurring', [
                'intent' => $request->user()->createSetupIntent(),
                'id'=>'prod_P7oJa9TgVsWPyM'
            ]);
        }
    }
    ...
    {
        Log::info("handleCustomerSubscriptionCreated called");
    ...
    {
        $res = Cashier::findBillable($stripeId);
