@if ($user->trashed() && $logged_in_user->hasAllAccess())
    ...
    @if (config('boilerplate.access.user.permanently_delete'))
        <x-teckiproadmin::utils.delete-button
            :href="route('admin.auth.user.permanently-delete', $user)"
            :text="__('Permanently Delete')" />
    @endif
@else
    @if ($logged_in_user->hasAllAccess() || $logged_in_user->can('admin.auth.user.edit'))
        <x-teckiproadmin::utils.view-button :href="route('admin.auth.user.show', $user)" />
        <x-teckiproadmin::utils.edit-button :href="route('admin.auth.user.edit', $user)" />
    @endif
    ...
    @if ($user->id !== $logged_in_user->id && !$user->isMasterAdmin() && $logged_in_user->hasAllAccess())
        <x-teckiproadmin::utils.delete-button :href="route('admin.auth.user.destroy', $user)" />
    @endif
